#!/usr/bin/env python3
"""
CodeRED Xenia RDR Multiplayer Crash Guard v5

Purpose:
- Work around the current multiplayer-entry crash long enough to identify whether
  it is caused by CodeRED netplay, Xenia CPU/JIT settings, patches/title updates,
  or the selected RDR disc/module.
- Does not require uploading the full Xenia build.
"""
...
def log(root: pathlib.Path, msg: str) -> None:
    ...
    line = f"[{stamp()}] {msg}"
    print(line)
    with (logs / "codered_rdr_crash_guard_v5.log").open("a", encoding="utf-8") as f:
        f.write(line + "\n")
...
def write_configs(root: pathlib.Path, exe: pathlib.Path | None, args: argparse.Namespace, target: pathlib.Path | None) -> pathlib.Path:
    ...
    log_path = root / "logs" / f"xenia_codered_v5_{args.mode}_{args.disc}_{args.variant}.log"
    selected_iface = args.interface or ("127.0.0.1" if args.host in {"127.0.0.1", "localhost"} else local_ipv4_guess())
    ...
    updates = {
        ...
        "Kernel": {
            "apply_title_update": apply_title_update,
    ...
    for d in dirs:
        ...
        for name in CONFIG_NAMES:
            ...
            if path.exists():
                bak = path.with_suffix(path.suffix + f".codered_v5_bak_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}")
                try:
                    shutil.copy2(path, bak)
                except OSError:
                    pass
            path.write_text(update_toml(original, updates), encoding="utf-8")
            log(root, f"Wrote v5 config: {path}")
    return log_path
...
def launch(args: argparse.Namespace) -> int:
    ...
    if args.mode == "private" and not start_private_host(root, args.host, args.port):
        ...
        return 4
    cmd = [str(exe), f"--log_file={log_path}", "--log_level=3", str(target)]
...
def collect(root: pathlib.Path) -> pathlib.Path:
    logs = root / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    send_dir = logs / "CODERED_SEND_THESE_V5"
    if send_dir.exists():
    ...
    summary.write_text(
        "Upload this folder or the ZIP next to it. Crash .dmp files are intentionally excluded.\n"
        f"Created: {stamp()}\nFiles copied: {copied}\n",
        encoding="utf-8",
    )
    out = logs / f"codered_rdr_small_logs_v5_{dt.datetime.now().strftime('%Y%m%d_%H%M%S')}.zip"
    with zipfile.ZipFile(out, "w", compression=zipfile.ZIP_DEFLATED) as z:
        for p in send_dir.rglob("*"):
            if p.is_file():
                z.write(p, p.relative_to(send_dir.parent))
    # Validate central directory immediately.
    try:
