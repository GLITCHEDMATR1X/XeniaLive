...
class SpirvBuilder : public spv::Builder {
 public:
  ...
  // Compatibility wrappers for newer glslang debug-name overloads.
  spv::Id makeStructType(const std::vector<spv::Id>& members,
                         const char* name) {
    std::vector<spv::StructMemberDebugInfo> member_debug_info(members.size());
    return spv::Builder::makeStructType(members, member_debug_info, name);
  }

  spv::Id makeImageType(spv::Id sampled_type, spv::Dim dim, bool depth,
                        bool arrayed, bool ms, unsigned sampled,
                        spv::ImageFormat format) {
    return spv::Builder::makeImageType(sampled_type, dim, depth, arrayed, ms,
                                       sampled, format, nullptr);
  }

  spv::Id makeSamplerType() {
    return spv::Builder::makeSamplerType(nullptr);
  }

  spv::Id makeSampledImageType(spv::Id image_type) {
    return spv::Builder::makeSampledImageType(image_type, nullptr);
  }
